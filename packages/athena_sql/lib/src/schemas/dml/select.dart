...
class SelectTableSchema extends DMLSchema {
  ...
  .keyword('SELECT ')
  .comaSpaceSeparated(columns.map((column) => QueryString().userInput(column)))
  .keyword(' FROM ')
  .userInput(from)
  .condition(as !=null, (q) => q.keyword(' AS ').userInput(as!))
  .condition(where != null, (q) => q.keyword(' WHERE ').adding(where!));

  SelectTableSchema copyWith({List<String>? columns, String? from, String? as, WhereClause? where}) {
