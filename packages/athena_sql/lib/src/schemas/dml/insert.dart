...
class InsertTableSchema extends DMLSchema {
  ...
  QueryPrintable build() {
    ...
    final valuesString = values.map((e) => keys.map((key) => e[key]).toList()).toList();
    

    return QueryString().keyword('INSERT')
      .keyword('INTO')
      .userInput(name)
      .parentheses((q) => q
        .commaSeparated(keys.map((e) => QueryString().userInput(e)))
      )
      .keyword('VALUES')
      .parentheses((q) => q
        .commaSeparated(valuesString.map((e) => QueryString().parentheses((q) => q
          .commaSeparated(e.map((e) => QueryString().userInput(e)).toList())
        )).toList())
      );
  }
