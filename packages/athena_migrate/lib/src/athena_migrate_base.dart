...
import 'package:athena_migrate/src/commands/migrate_up.dart';
...
class AthenaMigrateComands {
  ...
  Future<void> run(List<String> args) async {
    ...
    for (var element in executables) {
      ...
      if(args.isNotEmpty) {
        final key = args[0];
        final executable = executables.firstWhereOrNull((element) => element.command == key);
        if (executable == null) {
        ...
        final selected = menu(prompt: 'select an option', options: executables, format: (ex) => '${ex.command} - ${ex.description}');
